...
//发送文件  注意这里一定是const char*file不然会有错误提示；
...
//编码，用作回写浏览器的时候，将除数字字母 以及 ./-~以外的字符转义回写；
    ...
    for ( ; *from != '\0'; ++to, ++from  ) 
    {
        ...
        { 
            ...
            *to = hexit(from[1])*16 + hexit(from[2]);
			//移过已经处理的两个字符（%21指向1），表达式3的++from还会再向后移一个字符；
            from += 2;                      
        } 
        else
        {
